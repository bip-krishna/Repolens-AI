...
import { getRepoMetadata, getRepoTree, getFileContent } from "@/lib/github";
import { generateFeatureExtraction } from "@/lib/ai";
import { featureExtractionPrompt } from "@/lib/prompts";
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const prompt = featureExtractionPrompt(`${owner}/${repo}`, treeStr, packageJsonStr);
    const result = await generateFeatureExtraction(prompt);
