...
import { generateSecurityAnalysis } from "@/lib/ai";
import { securityAnalysisPrompt } from "@/lib/prompts";
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const prompt = securityAnalysisPrompt(`${owner}/${repo}`, treeStr, packageJsonStr);
    const result = await generateSecurityAnalysis(prompt);
